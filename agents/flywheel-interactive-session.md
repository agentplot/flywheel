...
an intent conductor) names the intent change, the task batch you work, and
...
one, report that to your conductor as the next batch's type rather than
switching channels inside your own run.
...
task is your conductor's own scope — books and the map — so a writeback

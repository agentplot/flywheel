...
- **hold** — stays queued on the bolt, explicitly out of this round.
...
order — commit, containers at Backlog, Ready flips last; a card approved
onto this open bolt flips Ready and the live loop expands it mid-flight
— then clear `needs-operator` and settle. Applying the plan drains the
...
route. A finding that needs no routing decision does not exist: hold is
a decision too. Machinery findings follow the finding-routing rule: your
report, never the tracker.

## What you report

One line per row with its applied route and link, what was held and why,
and whether the inbox is empty.

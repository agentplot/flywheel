...
A construction work item closes twice over, and the bolt loop does both —
...
reasons or neither. No recipe for it here on purpose: the upgrade is the
loop's one-call swap, and a session that made it by hand would race the
loop for bookkeeping that was never its to write.

...
- [ ] 1.1 Re-read `guard_scaffold`, `CHARTER_SECTIONS`, `guards()`,
      ...
      over it.
- [ ] 1.2 Confirm from `schemas/bolt-default/schema.yaml` and its three
...
- [ ] 2.1 `guard_scaffold` returns early only when
      ...
      guard.
- [ ] 2.2 The post-settle checks run on both paths: the change directory
      ...
      not one per path.
- [ ] 2.3 The dry-run branch reports which of the two it would do —
...
- [ ] 2.4 A pass over a record whose `bolt.md` exists drives nothing,
      writes nothing and appends no action, whatever the charter says.

## 3. The order for a change that already exists

- [ ] 3.1 The charter text the order carries — the four sections, the
...
- [ ] 3.2 The existing-change path's invocation is the one that adds a
...
- [ ] 3.3 The existing-change order names the bolt type the change is
      ...
      carries.
- [ ] 3.4 The session name is unchanged on both paths, so a charterless
      record found on a later pass resumes the scaffold session rather
      than starting a second one.

## 4. Tests

- [ ] 4.1 Replace `test_a_present_change_directory_returns_before_the_check`
      ...
      drives none.
- [ ] 4.2 A charterless change directory: the session is driven, the
      ...
      action.
- [ ] 4.3 A charterless change directory whose driven session writes no
...
- [ ] 4.4 A `bolt.md` present but carrying no readable criteria (the
      older shape, `# Unit:` section and all) drives no session, writes
      nothing, records no action, and is left for the landing's refusal.
- [ ] 4.5 `--dry-run` over a charterless change directory: one reported
      action naming the charter it would write, no session launched, no
      file written.
- [ ] 4.6 The two orders carry the same charter text — asserted on both,
      not on one.
- [ ] 4.7 `python3 -m unittest discover tests -v` green, and the repo's
      gates — `node scripts/check-paths.mjs`, `node scripts/check-site.mjs`,
      `bash scripts/validate-manifests.sh` — green.

## 5. Record

- [ ] 5.1 `openspec validate a-charterless-change-directory-gets-one --strict`

...
- [ ] 1.1 In `tests/test_derived_backlog.py`,
...
- [ ] 1.2 In `bin/_flywheel_bolt_loop.py`, correct the comment in the
...
- [ ] 2.1 Add the requirement "The operator's milestone close releases
...
- [ ] 2.2 Bring `landing_wanted`'s comment in `bin/_flywheel_bolt_loop.py`
...
- [ ] 2.3 In `holding_cards`'s docstring, keep "Asked before
...
- [ ] 3.1 In `tests/test_bolt_loop.py`, `LandingHoldTest.merged`: build
...
- [ ] 3.2 In `test_a_second_unit_expanded_later_does_not_buy_a_second_landing`,
...
- [ ] 4.1 Add a test to `tests/test_bolt_loop.py` covering the first
...
- [ ] 4.2 Add a test covering the forced-landing asymmetry: with the
...
- [ ] 4.3 Add a test covering the "both conditions outstanding" scenario:
...
- [ ] 5.1 `sh scripts/test.sh` green in this worktree — which is
...
- [ ] 5.2 The three repo gates green: `sh scripts/validate-manifests.sh`,
  `node scripts/check-paths.mjs`, `node scripts/check-site.mjs`.
  *Done when* all three exit 0.
- [ ] 5.3 `openspec validate matches-the-book-259 --strict` green, and

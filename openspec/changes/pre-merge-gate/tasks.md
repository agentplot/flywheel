...
- [ ] 1.2 Re-read `openspec/changes/merge-gate-remedy/bolt.md` for the
...
- [ ] 1.3 Confirm the co-landing sibling `openspec/changes/gate-prose-correction/`
...
- [ ] 2.4 Verify neither "during `wt merge` before the commit, on the exact
      ...
      execution.
- [ ] 2.5 Verify the block names the config-locus question as open, carries
...
- [ ] 2.7 **Produce the four-template measurement on the tree that lands.**
...
- [ ] 3.1 Run the three checks directly and confirm green:
...
- [ ] 3.2 `openspec validate --strict` green.
- [ ] 3.3 Record what could **not** be verified and why: `wt hook pre-merge`
...
- [ ] 4.1 Commit by pathspec (`git add -- <paths>`, then
...
- [ ] 4.2 Report the four hook template strings **verbatim**, with the
      worktree path and the commit sha, for the conductor to take to #33.
- [ ] 4.3 **From that report onward, nothing touches `.config/wt.toml`** —
      ...
      slip in.
- [ ] 4.4 Do not merge. Do not push. The merge-back is the conductor's, it

...
- [ ] 1.1 `skills/bolt-planning/SKILL.md`, the **Delivery** section's
...
- [ ] 1.2 The same file's **The plan documents** section names the bolt
...
- [ ] 1.3 `agents/flywheel-bolt-planner.md` carries the same contract in
...
- [ ] 1.4 No surface a planning run reads still tells it to title a
...
- [ ] 2.1 Add a test class to `tests/test_derived_backlog.py` that reads
      ...
      `closed:superseded`.
- [ ] 2.2 The same test asserts the retired proposition is gone: neither
...
- [ ] 2.3 The new test fails against the pre-change text of at least one
...
- [ ] 3.1 `sh scripts/test.sh` green — the whole suite, not just the new
      class.
- [ ] 3.2 `node scripts/check-paths.mjs` and `node scripts/check-site.mjs`
      green, per the project instructions in `openspec/config.yaml`.
- [ ] 3.3 `openspec validate planner-owns-the-milestone --strict` green.
- [ ] 3.4 Commit by pathspec (`git add -- <paths>`, `git commit -- <paths>`)

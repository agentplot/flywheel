...
- [ ] 1.1 Confirm `openspec/specs/` holds `flywheel-stage-labels`,
...
- [ ] 1.2 Confirm `bin/_flywheel_bolt_loop.py` still has `guard_stages`
...
- [ ] 1.3 Confirm `bin/flywheel-bolt-loop` still resolves the type as
      `args.type_name or binding.get("schema") or "bolt-quick"` and passes
      only the binding to `refuse_stage_declaration`.
- [ ] 1.4 Confirm `set_stage` in `bin/_flywheel_inbox.py` still returns
...
- [ ] 1.5 Confirm `Writer` in `bin/_flywheel_intent.py` still keeps `_added`
      and `_removed`, and that reassigning `writer.snapshot` after a re-read
      clears neither.
- [ ] 1.6 Confirm `skills/_reference/herdr.md` still gives the flip as a
      literal `gh issue edit … --remove-label stage:in-session --add-label
      stage:done`, and that `bin/` holds no stage command.
- [ ] 1.7 Confirm `compose_unit` in `bin/_flywheel_intent.py` is still called
      ...
      released assertions".
- [ ] 1.8 Confirm `server_inbox` in `bin/_flywheel_inbox.py` still adds a
...
- [ ] 3.1 In `bin/flywheel-bolt-loop`, refuse a `--type` that disagrees with
...
- [ ] 4.1 Add `bolt-direct` to `tests/test_bolt_loop.py`'s
      ...
      declares.
- [ ] 4.2 Make the cycle test that proves `bolt-direct` runs no verify load
...
- [ ] 4.3 Confirm the shipped stage set is covered too, not only strategy
...
- [ ] 5.1 Narrow `set_stage`'s early return to "already at the target **and**
...
- [ ] 5.2 Invalidate `Writer._added` wherever `_removed` is invalidated, so
      neither outlives the snapshot behind it. Reassigning
      `writer.snapshot` is the moment.
- [ ] 5.3 Add the command that makes the flip from a pane — one item, one
...
- [ ] 5.4 Replace the hand-built recipe in `skills/_reference/herdr.md` with
      that call, keeping the sentence that says a stage move replaces the
      previous one.
- [ ] 5.5 Follow the recipe's echoes: both agent profiles
...
- [ ] 5.6 Pin the case the hard-coded recipe got wrong: an item at
...
- [ ] 6.1 Apply the `flywheel-release-unit-parent` delta: the born-ready
...
- [ ] 6.2 Apply the bar-denominator correction. State what the bar counts on
      each path; do not compute a corrected figure, which the same
      capability forbids in "No second progress figure exists".
- [ ] 6.3 Confirm no neighbour is left disagreeing: the sibling requirement
...
- [ ] 7.1 Close the release's unit parent at the landing, with one
...
- [ ] 7.2 Touch no sub-issue while doing it: the assertions' closes belong
      to the merge boundary and the landing.
- [ ] 7.3 Give `server_inbox`'s Ready-batch branch the milestone-state test
...
- [ ] 7.4 Pin both: a landed bolt's parent is closed and its milestone
      reports no job; a Ready unit on an **open** milestone still starts its
      loop exactly as today.

## 8. The skipped-stage scenario

- [ ] 8.1 Apply the `flywheel-construction-stages` delta's skipped-stage
...
- [ ] 8.2 Confirm on disk that the boundary writes still test `ran` rather
...
- [ ] 9.1 Move `tests/test_inbox.py`'s `if __name__ == "__main__":` block to
...
- [ ] 9.2 Check every other file under `tests/` for the same misplacement
      and fix any found.
- [ ] 9.3 Add one `LandingTest` case on a **worktree-bearing** type
...
- [ ] 10.1 `#133` — `land_stage`'s item set is `[i for i in on_milestone if
...
- [ ] 10.7 `#146` — `read_binding` reads dash lists as well as scalars and
      flow lists, so a block-style `stages:` is refused rather than ignored.
- [ ] 10.8 `#148` and `#158` — `collect_plan` excludes an item carrying
...
- [ ] 10.9 `#149` and `#157` — `dispatch_batch` records the session name per
...
- [ ] 10.10 `#150` — `flywheel-stage-labels`' first requirement heading
...
- [ ] 10.11 `#151`'s second half and `#154`'s framing — `LoopConfig.validate`
      raises on an unknown stage name and is called at the entry point.
- [ ] 10.12 Comment the evidence on each of the thirteen items, naming the
...
- [ ] 11.1 `openspec validate stage-labels-133 --strict` green.
- [ ] 11.2 `sh scripts/test.sh` green, and the count higher than before by
...
- [ ] 11.3 `devenv shell -- gates` green: `sh scripts/validate-manifests.sh`,

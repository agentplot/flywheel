...
- [x] 1.1 Re-read `_predecessor_in` and `_expand_card` in
...
- [x] 1.2 Confirm from `guard_expand`'s call path — `cycle()` and whatever
...
- [x] 1.3 Re-read `TrackerSnapshot.from_fixture`, `FixtureTracker` and
...
- [x] 2.1 `_predecessor_in` takes the blocker's work items from the
...
- [x] 2.2 Each owned sub-issue is answered from the snapshot when the
...
- [x] 2.3 A blocker owning at least one sub-issue is "all in" exactly when
...
- [x] 2.4 The predicate returns, alongside its verdict, which state it
...
- [x] 2.5 Correct `_predecessor_in`'s docstring where it calls the tracker
      ...
      from visibility.
- [x] 2.6 `FixtureTracker` gains `sub_issues(number)`, returning the
...
- [x] 3.1 A `TrackerSnapshot` built in the live shape — an open
...
- [x] 3.2 The same shape with one child closed off the happy path and one
      child open and visible: the pass defers and writes nothing.
- [x] 3.3 A blocker whose members are all visible in the snapshot — every
...
- [x] 3.4 An expanded blocker owning no sub-issues defers, and the reason
      recorded says it has no work items — distinct from the reason
      recorded for an unexpanded blocker.
- [x] 3.5 A blocker closed off the happy path is settled: keep the
...
- [x] 3.6 The three existing `ExpansionTest` defer scenarios —
...
- [x] 3.7 The run record carries the reason: assert through the ledger note
...
- [x] 4.1 `openspec validate the-defer-predicate-reads-a-closed-unit

...
- [x] 1.1 In `bin/_flywheel_bolt_loop.py`, narrow `guard_expand`'s
...
- [x] 1.2 Drop `create_milestone` and the `set_milestone` branch from
...
- [x] 1.3 Rewrite `guard_expand`'s docstring over the unit shape — the
...
- [x] 1.4 Update `tests/test_derived_backlog.py::ExpansionTest`:
...
- [x] 2.2 Read the blocker's state from the snapshot the guard already
...
- [x] 2.3 Rewrite the docstring line "a card blocked by an issue not yet
...
- [x] 2.4 Replace `test_blocked_by_unlanded_predecessor_defers` and
...
- [x] 3.1 Add `guard_charter` to `BoltLoop`, ordered after
...
- [x] 3.2 Commit the charter write by pathspec on the branch the record
...
- [x] 3.3 Record the append in `actions` (so the loop's STOP condition
  stays honest) and in the ledger, naming the unit whose section landed.
- [x] 3.4 Narrow `guard_scaffold`'s work order line "If the milestone's
...
- [x] 4.1 Confirm the no-Team refusal comment in `_expand_card` says the
  **unit** is unroutable, and that the refusal leaves the bolt's other
  units untouched.
- [x] 4.2 Run the repo's gates — `sh scripts/test.sh`,
  ...
  green.
- [x] 4.3 Commit by pathspec: the change's artifacts, the loop, and the

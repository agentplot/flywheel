...
- [x] 1.1 Re-read `guard_scaffold`, `CHARTER_SECTIONS`, `guard_charter`,
      ...
      building over it.
- [x] 1.2 Confirm `schemas/bolt-default/templates/bolt.md` carries the
...
- [x] 2.1 `BoltParams` carries the milestone description and
      `bin/flywheel-bolt-loop` passes it the value it reads for
      `plan_mode_declared`; a loop built under `--fixture` holds the
      empty string.
- [x] 2.2 `guard_scaffold`'s order names the four charter sections, asks
...
- [x] 2.3 The post-settle check calls `merge_criteria()` — the same
      reader the landing uses, not a second regex — and fails with a
      reason naming the change directory and what is missing.
- [x] 2.4 `land_stage` refuses a charter that states no criteria, sitting
...
- [x] 3.1 All four `bolt-*` schemas declare a `unit` artifact generating
...
- [x] 3.2 The `bolt` instruction in all four members stops at the four
...
- [x] 3.3 `bash scripts/validate-manifests.sh` passes over the edited
      schemas and `bin/install-schemas` still copies each member whole.

## 4. The scaffold's order stops at the charter

- [x] 4.1 The "BELOW them: … copy the LOWEST-NUMBERED one's body into
...
- [x] 5.2 Its idempotency test is the record's committed state — which
...
- [x] 5.3 A torn write is repaired, not read as done: a unit file on disk
...
- [x] 5.4 A record in the older shape needs no migration step: because
...
- [x] 5.5 The guard still skips itself under `--dry-run` and under
...
- [x] 6.1 `merge_criteria()` reads the file up to the first `# Unit:`
...
- [x] 6.2 `landing_mode()` is unchanged: it reads a declared `Landing:`
      line on a charter that states one, and a charter that states none
      reaches the refusal rather than a default.

## 7. Tests

- [x] 7.1 The scaffold work order's content: it names the four sections
...
- [x] 7.2 The unit artifact: first unit written and committed; second
...
- [x] 7.3 A record in the older shape — `bolt.md` carrying a `# Unit:`
      ...
      refusal fire on it.
- [x] 7.4 The landing refusal reached through the real `merge_criteria()`

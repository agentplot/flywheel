...
- [x] 1.1 `PlanCard.bolt` answers the card's own `bolt/*` milestone and
...
- [x] 1.2 `PlanCard` carries the state of the milestone it sits on, the
...
- [x] 2.1 A Ready card yields the `run` job for `card.bolt` only when
...
- [x] 2.2 A card with no `bolt/*` milestone yields no job at all. No
      synthesized milestone name reaches the job list.
- [x] 2.3 The card's reason wins for its milestone: after the pass, a
...
- [x] 2.4 A card at Backlog still yields nothing, and the sweep's other
...
- [x] 3.1 Every open card at Backlog is listed under "waiting on the
...
- [x] 3.2 A card at Ready is not listed there: its milestone already
      appears in the job rows, with the card as the reason (task 2.3).
- [x] 3.3 A card at Ready carrying no `bolt/*` milestone is listed
      under "waiting on the operator" with that defect named — it
      yields no job, so this line is the only place it can surface.
- [x] 3.4 The block still reads only: no start, no tracker write, and
...
- [x] 4.1 The Ready-card test asserts the job's `why` names the card,
      not only the milestone.
- [x] 4.2 New: a Ready card with `milestone=None` (title unchanged)
      yields no job — the case that fails before task 1.1 and passes
      after.
- [x] 4.3 New: a Ready card on a closed milestone yields no `run` job,
      mirroring `test_a_ready_batch_on_a_closed_milestone_is_not_a_run_job`
      in `tests/test_inbox.py`.
- [x] 4.4 New: a milestone holding both a Ready card and a Ready batch
      yields one `run` job whose reason names the card.
- [x] 4.5 New: the status surface — cards at Backlog listed as waiting
      ...
      needs no tracker.
- [x] 4.6 Each new test fails against the pre-change code and passes
      after. A test that is green both ways holds nothing.

## 5. Gates

- [x] 5.1 `sh scripts/test.sh` green — the whole suite, not just the
      new tests.
- [x] 5.2 `node scripts/check-paths.mjs` and `node scripts/check-site.mjs`
      green, per the project instructions in `openspec/config.yaml`.
- [x] 5.3 `openspec validate the-bolt-job-filter --strict` green.
- [x] 5.4 Commit by pathspec (`git add -- <paths>`,

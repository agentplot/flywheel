...
| 2 | `expansion-makes-a-unit` | expansion relabels the approved card `unit`, consumes its Ready status, files one item per plan task as its sub-issues on the same milestone, and writes the plan into the bolt's charter; idempotent; refuses a card with no Team; defers a card whose blocking predecessor's work is not all in — the predecessor's merge, not its close | `books/flywheel/src/bolt-planning.md`, `books/flywheel/src/lifecycles.md` | 1 | a card that is already on its milestone cannot be expanded by a step that creates one |

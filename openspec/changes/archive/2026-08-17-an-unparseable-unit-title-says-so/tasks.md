...
- [x] 1.1 Re-read `guard_charter`'s unit scan and its `--dry-run`,
...
- [x] 1.2 Confirm from `guards()`, `cycle()` and `run()` that a reason
...
- [x] 1.3 Re-read `CharterTest`, `FakeGit`, `NotTheFixtureTracker` and the
...
- [x] 2.1 The unit scan collects, alongside the units it will write, every
...
- [x] 2.2 `guard_charter` returns a reason naming every such unit — each by
...
- [x] 2.3 The reason is returned on a pass that writes nothing of its own
      ...
      unit.
- [x] 2.4 On a pass that has nameable units to write, those files are
      ...
      on the bad one.
- [x] 2.5 A milestone whose unit titles all parse is untouched: the guard's
...
- [x] 2.6 Replace the scan's "left alone here" comment with what the code
      now does, and say why a pause rather than a log: the record, not the
      loop log, is where facts originate.

## 3. Tests

- [x] 3.1 A unit whose title parses no slug: the cycle halts, and the
...
- [x] 3.2 Two unnameable units on one milestone: one reason names both, in
      item order.
- [x] 3.3 One nameable unit beside an unnameable one: the nameable unit's
      file is written, committed by pathspec, and reaches HEAD, and the
      guard then returns the reason naming the other.
- [x] 3.4 A failed commit on the same pass: the commit-failure reason is
      what comes back, and the pass after the commit succeeds returns the
      unnameable unit's reason.
- [x] 3.5 `--dry-run` over an unnameable unit: the reason is among the
      reported actions and is returned, and no file is written and no
      commit is made.
- [x] 3.6 A fixture-tracker run over an unnameable unit: the reason is
      returned, and no `git add` or `git commit` runs against anyone's
      checkout.
- [x] 3.7 The pause reaches the operator: a guard reason returned from
...
- [x] 3.8 `python3 -m unittest discover tests -v` green, and the repo's
      gates — `node scripts/check-paths.mjs`, `node scripts/check-site.mjs`,
      `bash scripts/validate-manifests.sh` — green.

## 4. Record

- [x] 4.1 `openspec validate an-unparseable-unit-title-says-so --strict`

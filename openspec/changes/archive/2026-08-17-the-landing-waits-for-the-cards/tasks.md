...
- [x] 1.1 Re-read `Bolt.run`'s tail in `bin/_flywheel_bolt_loop.py` —
...
- [x] 2.1 An open unit card on this bolt's milestone holds the landing:
      ...
      is never in it.
- [x] 2.2 The hold outranks a forced landing: it is evaluated without
      consulting the `land` argument, so `land="force"` reports the hold
      rather than landing over it.
- [x] 2.3 A held run says so on its landing line: `RunReport.landing`
...
- [x] 2.4 The run record keeps the note it writes today — the ledger
...
- [x] 3.1 The landing closes **every** open unit on the bolt's
...
- [x] 4.1 In `tests/test_bolt_loop.py`, cover the hold: an open card at
      ...
      `not attempted`.
- [x] 4.2 Cover the several-units close: a milestone carrying two
...
- [x] 4.3 Cover the boundary: a second unit expanded after an earlier
...
- [x] 5.1 `sh scripts/test.sh` green.
- [x] 5.2 `devenv shell -- gates` green — `sh scripts/validate-manifests.sh`,
      `node scripts/check-paths.mjs`, `node scripts/check-site.mjs`.
- [x] 5.3 `openspec validate the-landing-waits-for-the-cards --strict`
